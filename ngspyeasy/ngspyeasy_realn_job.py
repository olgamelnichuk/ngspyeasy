...
def main(argv):
    ...
    try:
        ngspyeasy_realn_job(tsv_conf, projects_home, args.sample_id, args.task, args.uid, args.gid)
    except Exception as ex:
        logger().exception(ex)
        sys.exit(1)


def ngspyeasy_realn_job(tsv_conf, projects_home, sample_id, task, uid, gid):
