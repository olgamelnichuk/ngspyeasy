...
def run_fastqc(row, projects_home):
    ...
    try:
        output = subprocess.check_output(["/bin/bash", "-c", "env"], stderr=subprocess.STDOUT, shell=True,
                                         env=os.environ.copy())
        log_info("env: \n" + output)
        output = subprocess.check_output(["/bin/bash", "-c", "echo $CLASSPATH"], stderr=subprocess.STDOUT, shell=True,
                                         env=os.environ.copy())
        log_info("echo $CLASSPATH: \n" + output)
        output = subprocess.check_output(["/bin/bash", "-c", cmd], stderr=subprocess.STDOUT, shell=True,
                                         env=os.environ.copy())
        retcode = 0
