...
import unittest
import tempfile
from ngspyeasy import utils


class CommandLineOptionsTest(unittest.TestCase):
    def test_no_options(self):
        self.assertError(utils.check_cmdline_options(None, None))

    def test_projects_dir_is_none(self):
        self.assertError(utils.check_cmdline_options("config.tsv", None))

    def test_tsv_name_is_none(self):
        self.assertError(utils.check_cmdline_options(None, "/some_dir"))

    def test_projects_dir_not_exists(self):
        self.assertError(utils.check_cmdline_options("config.tsv", "/some_dir"))

    def test_tsv_config_not_exists(self):
        self.assertError(utils.check_cmdline_options("config.tsv", tempfile.gettempdir()))
    ...
    def test_all_options(self):
        ...
        (tsv_name, projects_home, errmsg) = utils.check_cmdline_options(file.name, ngs_projects_dir)
